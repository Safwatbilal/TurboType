...
export default function ContestUser({ contest }) {
  ...
  // دالة renderPagination لعرض الأزرار الخاصة بالترقيم
  const renderPagination = () => {
    const pages = [];
    const pageNumbersToDisplay = 3; // عدد الصفحات التي نعرضها حول الصفحة الحالية
    const minPage = Math.max(1, currentPage - pageNumbersToDisplay); // أول صفحة تظهر
    const maxPage = Math.min(totalPages, currentPage + pageNumbersToDisplay); // آخر صفحة تظهر
    ...
    <div className="text-[#646669] mb-6">
        ...
        <table className="min-w-full text-white border-collapse">
          <thead>
            <tr className="bg-[#2b2c2d] text-left">
              ...
              <th className="py-2 px-4 border-b border-[#2C2C2C]">Contest Status</th> {/* إضافة عمود حالة المسابقة */}
            </tr>
          </thead>
          <tbody>
              ...
              >
                ...
                <td className="py-2 px-4 border-b border-[#2C2C2C] text-[#646669]">
                  ...
                  {isContestOngoing(participant.contest_id.created_at) ? (
                    <span className="text-green-500">Ongoing</span> // حالة المسابقة جارية
                  ) : (
                    <span className="text-red-500">Ended</span> // حالة المسابقة منتهية
                  )}
                </td>
              </tr>
            ))}
      ...
      {totalPages > 1 && (
        <div className="flex justify-center mt-4 space-x-4">
          ...
          {/* عرض الأزرار بناءً على renderPagination */}
          {renderPagination()}
          <button
            onClick={nextPage}
